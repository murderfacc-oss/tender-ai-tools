...
- `mcp/` — Python-сервер MCP (server.py, zakupki_scraper.py, requirements.txt, .env.example)
...
**Шаг 2. Настроить .env с API-ключом**

Скопируй `mcp/.env.example` в `mcp/.env`. Затем спроси у меня:

> «Введи свой API-ключ DaMIA (получить на damia.ru, тариф API-Старт):»

Дождись ответа, открой `mcp/.env` и замени строку
`DAMIA_API_KEY=your_api_key_here` на полученный ключ. **Никогда не пиши ключ
в чат-логи и не публикуй его.**

Если у меня ещё нет ключа — оставь файл с заглушкой и скажи: «Зарегистрируйся
на damia.ru → API-Закупки → тариф API-Старт. Получишь ключ — вставь в
mcp/.env вместо your_api_key_here».
...
> Открой Cowork (https://cowork.anthropic.com — уточни URL у партнёра).
...
- ✅ что готово
- ⏳ что осталось сделать вручную (вставить API-ключ, если ещё не сделано;
  загрузить скиллы в Cowork; перезапустить Claude Code)
...
4. Отвечай на вопросы Claude Code, когда он попросит API-ключ.
